...
// BlackbodyToXYZ computes the CIE XYZ tristimulus values of a blackbody
// radiator at temperature T (in Kelvin) by numerical integration of
// Planck's spectral radiance weighted by the CIE 1931 2-degree standard
// observer color matching functions.
//
// The integration uses a 5nm step from 380nm to 780nm with tabulated
// CIE observer data.
//
// The result is normalized so that Y=1 for a perfect white diffuser
// (i.e., the Y value represents relative luminance, not absolute).
//
// Valid range: T > 0 (Kelvin); practical range ~1000K to ~25000K
// Precision: limited by 5nm integration step and tabulated data
// Reference: Planck, M. (1901); CIE 15:2004 (standard observer data)
func BlackbodyToXYZ(T float64) (X, Y, Z float64) {
	...
	for i := 0; i < len(cieObserver); i++ {
		lambda := cieObserver[i][0] * 1e-9 // nm to m
		xBar := cieObserver[i][1]
		yBar := cieObserver[i][2]
		zBar := cieObserver[i][3]

		// Planck spectral radiance (proportional; constants cancel in normalization)
		exp := h * c / (lambda * k * T)
		if exp > 709 { // avoid overflow in math.Exp
			continue
		}
		planck := 1.0 / (math.Pow(lambda, 5) * (math.Exp(exp) - 1))
...
// Reference: Reinhard, E. et al. (2002) "Photographic Tone Reproduction
// for Digital Images", ACM SIGGRAPH 2002
func ToneMapReinhard(r, g, b, whitePoint float64) (ro, go_, bo float64) {
	wp2 := whitePoint * whitePoint
	reinhard := func(v float64) float64 {
		return v * (1 + v/wp2) / (1 + v)
	}
	ro = reinhard(r)
	go_ = reinhard(g)
	bo = reinhard(b)
...
// cieObserver contains the CIE 1931 2-degree standard observer color
// matching functions at 5nm intervals from 380nm to 780nm.
// Format: [wavelength_nm, x_bar, y_bar, z_bar]
// Source: CIE 15:2004, Table 1
var cieObserver = [81][4]float64{
